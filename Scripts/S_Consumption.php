...
function PrintConsumptionsList(mysqli $conn, $Provider_ID, $id_client){
    ...
    if ($stmt_result->num_rows > 0) {
        ...
        do {
            ...
            echo '<form action="../Scripts/S_Consumption.php?Provider_ID='. $Provider_ID .'&Client_ID=' .$id_client. '&Facture_ID=' .$Facture['ID']. '" method="POST"></option>';
            echo '<td data-label="Consommation Real"><input class="input_RealConsumption" type="number" name="RealConsumption" required> KWH</td>';
...
function ConfirmeConsumption(mysqli $conn, $Facture_ID, $Client_ID){
    ...
    if (($conn->query($stmt_UpdateVerified) === TRUE) AND ($conn->query($stmt_UpdateDept) === TRUE))
        header("Location: ../Provider/ConsumptionVerification.php?id=" .$Provider_ID);
